...
namespace System.Runtime.Serialization.Formatters.Binary
{
    ...
    internal sealed class WriteObjectInfo
    {
        ...
        internal void InitSerialize(object obj, ISurrogateSelector? surrogateSelector, StreamingContext context, SerObjectInfoInit serObjectInfoInit, IFormatterConverter converter, ObjectWriter objectWriter, SerializationBinder? binder)
        {
            ...
            else if (obj is ISerializable)
            {
                ...
                {
                    throw new SerializationException(String.Format(SR.Serialization_NonSerType, _objectType.FullName, _objectType.Assembly.FullName));
                }
                _si = new SerializationInfo(_objectType, converter);
    ...
    internal sealed class ReadObjectInfo
    {
        ...
        internal MemberInfo? GetMemberInfo(string? name)
        {
            ...
            if (_isSi)
            {
                throw new SerializationException(String.Format(SR.Serialization_MemberInfo, _objectType + " " + name));
            }

            if (_cache._memberInfos == null)
            {
                throw new SerializationException(String.Format(SR.Serialization_NoMemberInfo, _objectType + " " + name));
            }
        ...
        internal Type? GetType(string name)
        {
            ...
            if (type == null)
            {
                throw new SerializationException(String.Format(SR.Serialization_ISerializableTypes, _objectType + " " + name));
            }
        ...
        internal Type[]? GetMemberTypes(string[] inMemberNames, Type? objectType)
        {
            if (_isSi)
            {
                throw new SerializationException(String.Format(SR.Serialization_ISerializableTypes, objectType));
            }
            ...
            for (int i = 0; i < _cache._memberInfos.Length; i++)
            {
                ...
                else
                {
                    ...
                    if (!isFound)
                    {
                        ...
                        {
                            Debug.Assert(_cache._memberNames != null);
                            throw new SerializationException(String.Format(SR.Serialization_MissingMember, _cache._memberNames[i], objectType, typeof(OptionalFieldAttribute).FullName));
                        }
                    }
                }
            }
        ...
        internal Type GetMemberType(MemberInfo objMember)
        {
            ...
            throw new SerializationException(String.Format(SR.Serialization_SerMemberInfo, objMember.GetType()));
        }
